...
from code_keras.idea3.loaddadaset import load_itemGenres_as_matrix
from code_keras.idea3.loaddadaset import load_negative_file
from code_keras.idea3.loaddadaset import load_rating_file_as_list
from code_keras.idea3.loaddadaset import load_rating_train_as_matrix
from code_keras.idea3.loaddadaset import load_user_attributes
from code_keras.idea3.evaluateml import evaluate_model
import code_keras.idea3.ml100k_pre as uicf
...
def get_lCoupledCF_model(num_users, num_items):
    ...
    # Input
    user_gender_input = Input(shape=(3,), dtype='float32', name='user_gender_input')  # 用户属性信息
    user_age_input = Input(shape=(62,), dtype='float32', name='user_age_input')
    user_oc_input = Input(shape=(23,), dtype='float32', name='user_0c_input')
    ...
    user_age_embedding = Dense(21, activation="relu", name="user_age_embedding1_ui")(user_age_input)  # 1st hidden layer
    user_age_embedding = Dense(6, activation="relu", name="user_age_embedding2_ui")(user_age_embedding)
    ...
    # item_attr_embedding = Dense(4, activation="relu", name="item_att_embedding_ui")(item_attr_embedding)
    ...
    user_latent_vector = user_id_Embedding(user_id_input)
    ...
    topLayer = Dense(1, activation='sigmoid', init='lecun_uniform',
                     name='topLayer')(attr_1)

    # Final prediction layer
    model = Model(
        input=[user_gender_input, user_age_input, user_oc_input, item_attr_input, user_id_input, item_id_input],
        output=topLayer)
...
def load_pretrain_model(model, gmf_model):
    ...
    gmf_user_age_embeddings_1 = gmf_model.get_layer('user_age_embedding1').get_weights()
    ...
    model.get_layer('user_age_embedding1_ui').set_weights(gmf_user_age_embeddings_1)
...
def main():
    ...
    mf_pretrain = ""
    # mlp_pretrain = ""
    # mf_pretrain = "Pretrain/ml_100k_ui_cf_32_1604462658.h5"

    evaluation_threads = 1
    startTime = time()
    model_out_file = 'Pretrain/%s_movieCF100k_%d_%s_%d.h5' % (dataset, num_factor, 5, time())
    # load data
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % verbose == 0:
            (hits, ndcgs) = evaluate_model(model, testRatings, testNegatives,
                                           user_gender_mat, user_age_mat, user_oc_mat, items_genres_mat, topK,
                                           evaluation_threads)
            hr, ndcg, loss5 = np.array(hits).mean(), np.array(ndcgs).mean(), hist5.history['loss'][0]
            print('Iteration %d [%.1f s]: HR = %.4f, NDCG = %.4f, loss5 = %.4f [%.1f s]'
                  % (epoch, t2 - t1, hr, ndcg, loss5, time() - t2))
            if hr > best_hr:
                best_hr = hr
                if out > 0:
                    model.save_weights(model_out_file, overwrite=True)
