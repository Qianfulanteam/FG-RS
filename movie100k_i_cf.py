...
def get_lCoupledCF_model(num_users, num_items):
    ...
    user_id_Embedding = Embedding(input_dim=num_users, output_dim=128, name='user_id_Embedding_ui',
    ...
    item_id_Embedding = Embedding(input_dim=num_items, output_dim=128, name='item_id_Embedding_ui',
                                  embeddings_initializer=RandomNormal(
                                      mean=0.0, stddev=0.01, seed=None),
                                  W_regularizer=l2(0), input_length=1)
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        user_gender_input, user_age_input, user_oc_input, user_id_input, item_attr_input, item_id_input, labels = get_train_instances(
            ...
            ratings,
            items_genres_mat)
        hist5 = model.fit([user_gender_input, user_age_input, user_oc_input, item_attr_input, user_id_input, item_id_input], labels, epochs=1,
                          batch_size=256, verbose=2, shuffle=True)
        t2 = time()
        # Evaluation
        if epoch % verbose == 0:
            (hits, ndcgs, recalls) = evaluate_model(model, testRatings, testNegatives,
                                           user_gender_mat, user_age_mat, user_oc_mat, items_genres_mat, topK, evaluation_threads)
