...
import code_keras.idea3.tafengpre as uicf
...
def get_lCoupledCF_model(num_users, num_items):
    ...
    user_age_embedding = Dense(6, activation="relu", name="user_age_embedding_ui")(user_age_input)  # 1st hidden layer
    user_age_embedding = Reshape((1, 6))(user_age_embedding)

    user_region_embedding = Dense(6, activation="relu", name="user_region_embedding_ui")(user_region_input)
    user_region_embedding = Reshape((1, 6))(user_region_embedding)


    item_sub_class_input = Input(shape=(1,), dtype='float32')

    item_sub_class = Embedding(input_dim=2012, output_dim=2, name='item_sub_class_ui',
                               embeddings_initializer=RandomNormal(
                                   mean=0.0, stddev=0.01, seed=None),
                               W_regularizer=l2(0), input_length=1)(item_sub_class_input)

    item_asset_price_input = Input(shape=(2,), dtype='float32')
    item_asset_price = Dense(4, activation="relu", name="item_asset_price_embedding_ui")(item_asset_price_input)
    item_asset_price = Reshape((1, 4))(item_asset_price)
    ...
    #u_i_region = Position_Embedding()(u_i_region)
    u_i_region = Self_Attention(2, 3)([u_i_region, u_i_region, u_i_region])
    u_i_region = GlobalAveragePooling1D()(u_i_region)
...
def load_pretrain_model(model, gmf_model):
    # MF embeddings
    gmf_user_att_embeddings = gmf_model.get_layer('user_age_embedding').get_weights()
    gmf_user_attention_probs_u = gmf_model.get_layer('user_region_embedding').get_weights()
    gmf_attention_u = gmf_model.get_layer('item_sub_class').get_weights()
    gmf_z_u_embedding = gmf_model.get_layer('item_asset_price_embedding').get_weights()



    model.get_layer('user_age_embedding_ui').set_weights(gmf_user_att_embeddings)
    model.get_layer('user_region_embedding_ui').set_weights(gmf_user_attention_probs_u)
    model.get_layer('item_sub_class_ui').set_weights(gmf_attention_u)
    model.get_layer('item_asset_price_embedding_ui').set_weights(gmf_z_u_embedding)
...
def main():
    ...
    out=1
    mf_pretrain = ""
    #mf_pretrain = "Pretrain/tafeng_uicf_9_1604577904.h5"

    startTime = time()
    model_out_file = 'Pretrain/%s_tafengUICF_%d_%d.h5' % (dataset, num_factor, time())
    # load data
    ...
    to_file = 'model_' + "tangfeng_uicf" + '.png'
    ...
    for epoch in range(num_epochs):
        ...
        user_age_input, user_region_input, user_id_input, item_attr_input, item_id_input, labels = get_train_instances(
            users_age_mat, user_region_mat,
            ratings,
            items_genres_mat)
